class AppWindow{
  ...
  setAppTitle(elmnt, title){
    document.querySelector(elmnt + ' div.appwindowheader span.app-title').innerText = title;
  }
  ...
  fullscreenOnApp(elmnt){
    ...
    document.querySelector(elmnt_query + " .appwindowheader").style.borderRadius = '0px';
    document.querySelector(elmnt_query + ' section.ui').style.height = '100%';
    document.querySelectorAll(elmnt_query + ' div.resizer').forEach((element) => {
  ...
  async fullscreenOffApp(elmnt){
    ...
    document.querySelector(elmnt_query + " .appwindowheader").style.borderRadius = '10px 10px 0px 0px';
    document.querySelector(elmnt_query + ' .ui').style.height = document.querySelector(elmnt_query + ' .ui').getAttribute('previous-height');
    document.querySelectorAll(elmnt_query + ' div.resizer').forEach((element) => {
  ...
  resizeHorizontal(elmnt, direction) {
    ...
    document.onmousemove = (e) => {
      ...
      if (direction == 'right'){
        var width = (Number(elmnt_width.replace('px', '')) - (mouseX - e.clientX));
      } else {
        var width = (Number(elmnt_width.replace('px', '')) + (mouseX - e.clientX));
      }
      ...
      if (width >= Number(elmnt.getAttribute('min-width'))){
        if (direction == 'right'){
          elmnt.style.left = ((Number(elmnt_left.replace('px', '')) - ((mouseX - e.clientX)/2)) ) + 'px';
        } else {
          elmnt.style.left = ((Number(elmnt_left.replace('px', '')) - ((mouseX - e.clientX)/2)) ) + 'px';
        }
  ...
  resizeVertical(elmnt) {
    ...
    document.onmousemove = (e) => {
      ...
      var height = (Number(elmnt_height.replace('px', '')) - (mouseY - e.clientY));
      
      console.log(height + 'px');
      
      if (height >= Number(elmnt.getAttribute('min-height'))){
        elmnt.style.top = ((Number(elmnt_top.replace('px', '')) - ((mouseY - e.clientY)/2)) ) + 'px';
        elmnt.style.height = (height + 'px');
        document.querySelector(elmnt_query + ' section.ui').setAttribute('height', (elmnt.clientHeight - 60) + 'px')
        document.querySelector(elmnt_query + ' section.ui').style.height = (elmnt.clientHeight - 60) + 'px';
      }
    };
  ...
  dragElement(elmnt) {
    ...
    if (document.querySelector(elmnt_query + " .appwindowheader")) {
      ...
      document.querySelector(elmnt_query + " .appwindowheader").onmousedown = dragMouseDown;
      document.querySelector(elmnt_query + " .appwindowheader").ondblclick = function () {toggleAppFullscreen(elmnt_query)};
    } else {
      /* otherwise, move the DIV from anywhere inside the DIV:*/
      elmnt.onmousedown = dragMouseDown;
      elmnt.ondblclick = function () {toggleAppFullscreen(elmnt_query)};
    }
    ...
    function fullscreenOnApp(elmnt){
      ...
      elmnt.style.borderRadius = '0px';
      document.querySelector(elmnt_query + " .appwindowheader").style.borderRadius = '0px';
      if (document.querySelector(elmnt_query + ' .ui').style.height != '-webkit-fill-available'){
        document.querySelector(elmnt_query + ' .ui').setAttribute('previous-height', document.querySelector(elmnt_query + ' .ui').style.height);
      }
      document.querySelector(elmnt_query + ' .ui').style.height = '-webkit-fill-available';
      document.querySelectorAll(elmnt_query + ' div.resizer').forEach((element) => {
        element.style.display = 'none';
      });
    }
